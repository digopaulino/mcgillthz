...
# Check this function
def get_T(ref_td, ref_fft, samp_td, samp_fft, freqs_for_fit=[2, 4]):
    """
    ...
    freqs_for_fit (list): List containing the minimum and maximum frequencies for fitting.
...
def minimize_err_at_freq(n_til, freq, T_function, T_function_pars, exp_amp, exp_phase, method='L-BFGS-B'):
    """
    ...
    exp_amp (float): Experimental amplitude.
    exp_phase (float): Experimental phase.
    method (str): Optimization method. Options include L-BFGS-B, Powell or Nelder-Mead
...
def minimize_err(n_til0, freqs, T_function, T_function_pars, exp_freqs, exp_amp, exp_phase, method='L-BFGS-B', start_from_low=False):
    """
    ...
    method (str): Optimization method. Options include L-BFGS-B, Powell or Nelder-Mead
                    See scipy.optimize.minimize function for explanation.
    start_from_low (bool): Whether to start optimization from the lowest frequency.
    ...
    result = minimize_err_at_freq(n_til0, func(freqs), T_function, T_function_pars, amp0, phase0, method=method)
    ...
    if not result.success:
        ...
        for i in range(len(freqs)):       
            ...
            result = minimize_err_at_freq(result.x[0] + 1j * result.x[1], freqs[j], T_function, T_function_pars, amp, phase, method=method)
            n_tils[j] = result.x[0] + 1j * result.x[1]

        return n_tils

def get_total_err(n_tils, freqs, T_function, T_function_pars, exp_freqs, exp_amps, exp_phases):
    """
