...
if 'temp_scans' not in st.session_state:
    st.session_state.temp_scans = {}  # Format: {temp_float: THzExp_object}
...
# --- HELPER FUNCTIONS ---
def select_folder():
    """Opens a native OS dialog to select a directory."""
    script = """
...
root.withdraw()
folder_path = filedialog.askdirectory(title="Select Folder with Temperature Scans")
print(folder_path)
"""
...
def extract_temperature(filename):
    """Extracts temperature from a filename like '196K_AB.csv' or '196 K_AB.csv'."""
...
def get_continuous_colors(temps, cmap_name):
    """Samples a continuous Plotly colorscale based on normalized temperatures."""
    if len(temps) <= 1:
        return ['#d62728'] # default red if only 1 temp
    
    # Map user names to Plotly Sequential names
    cmap_map = {
        'magma': 'Magma', 'thermal': 'Thermal', 'cividis': 'Cividis', 
        'hot': 'Hot', 'solar': 'Solar', 'sunset_r': 'Sunsetdark', 'agsunset': 'Agsunset'
    }
    plotly_cmap = cmap_map.get(cmap_name.lower(), 'Magma')
...
def plotly_spectrum(data_arr, fft_arr, color, label):
    """Helper to return traces for the 1D FFT overlaid maps."""
...
# ==========================================
with tab1:
    st.header("1. Batch Import Directory")
    col_btn, col_path = st.columns([1, 4])
    with col_btn:
        if st.button("📁 Select Folder", width="stretch"):
            folder_path = select_folder()
            if folder_path:
                st.session_state.folder_path = folder_path
                
                # 1. Find all AB files to extract base names
                all_files = os.listdir(folder_path)
                bases = set()
                for f in all_files:
                    if f.endswith('_AB.csv'):
                        bases.add(os.path.join(folder_path, f.replace('_AB.csv', '')))
                
                if not bases:
                    st.error("No valid dataset files (*_AB.csv) found in this directory.")
                else:
                    ...
                    if os.path.exists(config_path):
                        ...
                        st.warning("No `analysis_config.json` found in folder. Using fallback default processing parameters.")
                        cfg = {
    ...
    if st.session_state.sorted_temps:
        ...
        selected_raw_temp = c_sel.selectbox("Inspect Raw Dataset (Temperature K):", options=st.session_state.sorted_temps, key="tab1_temp")
...
with tab2:
    if not st.session_state.sorted_temps:
        ...
        sel_tau = st.select_slider("Excitation Delay τ (ps)", options=taus, value=taus[0], key='tab2_tau')
        col_match = [c for c in scan.A.columns[1:] if abs(float(c) - sel_tau) < 1e-5][0]
        
        fig_1d = make_subplots(rows=1, cols=2, subplot_titles=[f"Time Domain (τ = {sel_tau} ps)", f"Spectrum (τ = {sel_tau} ps)"])
        tr_tb, tr_fb = plotly_spectrum(np.array([scan.B['time'], scan.B[col_match]]), np.array([scan.fft_B['freq'], scan.fft_B[col_match]]), 'blue', 'B')
        tr_ta, tr_fa = plotly_spectrum(np.array([scan.A['time'], scan.A[col_match]]), np.array([scan.fft_A['freq'], scan.fft_A[col_match]]), 'red', 'A')
        ...
        fig_1d.update_xaxes(title_text="Frequency (THz)", range=[0, 6], row=1, col=2)
        fig_1d.update_yaxes(title_text="FFT Amplitude", type="log" if log_scale else "linear", row=1, col=2)
        fig_1d.update_layout(height=400, hovermode="x unified", template="plotly_white", margin=dict(l=0, r=0, t=30, b=0))
        ...
        with c_p1:
            ...
            fig2d.update_layout(title="2D Frequency-Frequency Map", xaxis_title="Probe frequency ν_t (THz)", yaxis_title="Excitation frequency ν_τ (THz)", height=500, template="plotly_white", margin=dict(l=0, r=0, t=30, b=0))
            fig2d.update_xaxes(range=[0, max_p_freq]); fig2d.update_yaxes(range=[-max_e_freq, max_e_freq])
            st.plotly_chart(fig2d, width="stretch")
            
        # Render Time-Freq Map
        ...
        with c_p2:
            ...
            figtf.update_layout(title="Time-Frequency Map (Probe FFT)", xaxis_title="Probe frequency ν_t (THz)", yaxis_title="Excitation delay τ (ps)", height=500, template="plotly_white", margin=dict(l=0, r=0, t=30, b=0))
...
with tab3:
    if not st.session_state.sorted_temps:
        ...
        # Ensure we grab a valid tau from the first scan
        ...
        for i, T in enumerate(st.session_state.sorted_temps):
            ...
            fig_ov_1d.add_trace(go.Scatter(x=fft_df['freq'], y=fft_df[col_m], name=f"{T} K", line=dict(color=temp_colors[i], width=2)))
            
        fig_ov_1d.update_xaxes(title_text="Frequency (THz)", range=[0, 6])
        fig_ov_1d.update_yaxes(title_text="FFT Amplitude (a.u.)", type="log" if ov_log else "linear")
        fig_ov_1d.update_layout(height=450, hovermode="x unified", template="plotly_white", margin=dict(l=0, r=0, t=30, b=0))
        st.plotly_chart(fig_ov_1d, width="stretch")
        
        st.divider()
        st.subheader("2D Cross Sections Overlay")
        ...
        with col_cs_plot:
            ...
            for i, T in enumerate(st.session_state.sorted_temps):
                ...
                if x_arr is not None:
                    fig_ov_cs.add_trace(go.Scatter(x=x_arr, y=y_arr, name=f"{T} K", line=dict(color=temp_colors[i], width=2)))
            ...
            if not fig_ov_cs.data:
                ...
                fig_ov_cs.update_layout(xaxis_title=x_lbl, yaxis_title=f"{cs_y_axis} (a.u.)", height=500, template="plotly_white", hovermode="x unified", margin=dict(l=0, r=0, t=10, b=0))
