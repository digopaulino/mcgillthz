...
def auto_correct_dataset(base_path, threshold, scan_orig=None):
    ...
    This allows it to be called from both the batch-loader and the re-evaluator.
    """
    if scan_orig is None:
        scan_orig = THzExp(base_path, shift_times=False)
    ...
    if len(flip_indices) == 0:
        ...
        # Attempt Type 2
        scan_t2 = THzExp(base_path, shift_times=False)
        ...
        if len(rem_indices) == 0:
            ...
            # Fallback to Type 1
            scan_t1 = THzExp(base_path, shift_times=False)
            for i, t in zip(flip_indices, flip_types):
...
if st.sidebar.button("📁 Browse & Process Flips", type="primary", use_container_width=True):
    ...
    if chosen_paths:
        ...
        st.session_state.datasets = {} # Clear old state
        ...
        st.session_state.review_select = None # Reset selection
...
if st.sidebar.button("💾 Save All Files", use_container_width=True):
    if not st.session_state.datasets:
        ...
        saved_count = 0
        with st.spinner("Backing up raw files and saving all datasets..."):
                    ...
                    try:
                        for ch_name, df in channels.items():
                            ...
                            if os.path.exists(orig_file):
                                shutil.copy(orig_file, os.path.join(raw_dir, os.path.basename(orig_file)))
        ...
        if saved_count > 0:
            st.sidebar.success(f"Successfully backed up and saved {saved_count} datasets!")
            st.balloons()
        else:
...
if not st.session_state.dataset_list:
    ...
    # 1. Build friendly display names (With Emojis to highlight changes)
    base_mapping = {}
        ...
        if col2.button("Re-detect & Correct", use_container_width=True):
            with st.spinner("Re-evaluating dataset..."):
                try:
                    ...
                    # Update display name locally so the dropdown doesn't lose our place
