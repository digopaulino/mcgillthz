...
KEYS_TO_SAVE =[
    ...
    'fft2d_sub_baseline', 'fft2d_Nt', 'fft2d_Ntau'
]
...
def plotly_2x2_grid(scan, title, min_t=None, max_t=None, vmax=0.1):
    ...
    fig.update_layout(
        title_text=title, height=750, template="plotly_white",
    ...
    if min_t and max_t:
        fig.update_xaxes(range=[min_t, max_t])
...
def plotly_2d_map_simple(df, title, min_t=None, max_t=None):
    t = df['time'].values
    ...
    fig.update_layout(title=title, xaxis_title="THz time t (ps)", yaxis_title="Excitation delay τ (ps)", height=550, template="plotly_white")
    if min_t and max_t:
        fig.update_xaxes(range=[min_t, max_t])
    return fig

def plotly_2d_fft(ft_data, freq_t, freq_tau, title, vmin=0, vmax=3, cmap='magma', log_scale=False, value_to_plot='Amplitude', plot_guidelines=True, cs_lines=None):
    ...
    if plot_guidelines:
        ...
        fig.add_trace(go.Scatter(x=[0, 3], y=[0, 0], mode='lines', line=line_kws, hoverinfo='skip', showlegend=False))
        fig.add_trace(go.Scatter(x=[0, 3], y=[0, 3], mode='lines', line=line_kws, hoverinfo='skip', showlegend=False))
        fig.add_trace(go.Scatter(x=[0, 3], y=[0, -3], mode='lines', line=line_kws, hoverinfo='skip', showlegend=False))
        fig.add_trace(go.Scatter(x=[0, 3], y=[0, 6], mode='lines', line=line_kws, hoverinfo='skip', showlegend=False))

    # User defined cross section lines (bounded to the dataset arrays safely)
    if cs_lines is not None:
        t_min, t_max = 0.0, min(3.0, freq_t.max())
        ...
        for line in cs_lines:
            if line.get('is_vertical', False):
                ...
                vt = np.linspace(t_min, t_max, 500)
                vtau = line['m'] * vt + line['b']
                # Mask out segments extending beyond freq_tau limits
    ...
    fig.update_layout(title=f"{title} ({value_to_plot})", xaxis_title="Probe frequency ν_t (THz)", yaxis_title="Excitation frequency ν_τ (THz)", height=650 if not cs_lines else 450, template="plotly_white")
...
def plotly_cross_sections(scan, cs_lines, x_axis_type, y_axis_type):
    fig = go.Figure()
    if not cs_lines:
        fig.update_layout(title="No lines added. Use controls to add a cross section.", template="plotly_white")
        return fig
    ...
    for line in cs_lines:
        ...
        # Skip Vertical lines if X-Axis is set to Probe frequency
        if is_vert and x_axis_type == "Probe Frequency (ν_t)":
            continue
    ...
    fig.update_layout(title=f"1D Cross Sections ({y_axis_type})", xaxis_title=x_label, yaxis_title=f"{y_axis_type} (a.u.)", height=450, template="plotly_white", hovermode="x unified")
    return fig
...
with tab1:
    ...
    if st.button("Load and Plot Grid", type="primary"):
        if not st.session_state.base_name:
            ...
            with st.spinner("Loading data..."):
                ...
                # --- CONFIG LOAD OR DYNAMIC DEFAULT GENERATION ---
...
with tab2:
    ...
    if st.session_state.scan is None:
        ...
        if st.button("Compute Individual FFTs"):
...
with tab3:
    ...
    if st.session_state.scan is None:
        ...
        c1, c2, c3, c4 = st.columns(4)
        st.session_state['leak_tau_min'] = c1.number_input("tau_min", value=float(st.session_state['leak_tau_min']))
        st.session_state['leak_tau_max'] = c2.number_input("tau_max", value=float(st.session_state['leak_tau_max']))
        st.session_state['leak_t_min'] = c3.number_input("t_min", value=float(st.session_state['leak_t_min']))
        st.session_state['leak_t_max'] = c4.number_input("t_max", value=float(st.session_state['leak_t_max']))
        
        if st.button("Subtract Leak"):
            scan = st.session_state.scan
        ...
        with colA:
            fig1 = plotly_2d_map_simple(st.session_state.scan.NL, "Before: Raw NL")
            ...
            fig1.add_trace(go.Scatter(x=x_corners, y=y_corners, mode='lines', line=dict(color='lime', width=2, dash='dash'), name='Averaging Region', hoverinfo='skip'))
            st.plotly_chart(fig1, use_container_width=True)
            
        with colB:
            if hasattr(st.session_state.scan, 'no_bg'):
                fig2 = plotly_2d_map_simple(st.session_state.scan.no_bg, "After: Leak Subtracted NL")
...
with tab4:
    ...
    if st.session_state.scan is None or not hasattr(st.session_state.scan, 'no_bg'):
        ...
        if st.button("Apply Window"):
            scan = st.session_state.scan
            with st.spinner("Applying window..."):
                ...
                st.session_state.scan = scan
                st.success("Window applied successfully!")

        if hasattr(st.session_state.scan, 'windowed'):
            colA, colB = st.columns(2)
            with colA:
                fig1 = plotly_2d_map_simple(st.session_state.scan.no_bg, "Before: Leak Subtracted NL")
                st.plotly_chart(fig1, use_container_width=True)
            with colB:
                fig2 = plotly_2d_map_simple(st.session_state.scan.windowed, "After: Windowed NL")
                st.plotly_chart(fig2, use_container_width=True)
...
with tab5:
    ...
    if st.session_state.scan is None or not hasattr(st.session_state.scan, 'windowed'):
        ...
        with col_controls:
            ...
            st.session_state['fft2d_sub_baseline'] = st.checkbox("Subtract Baseline", value=bool(st.session_state['fft2d_sub_baseline']))
            ...
            compute_plot_btn = st.button("Compute FFT", type="primary", use_container_width=True)
            
            st.divider()
            st.subheader("Plotting")
            value_to_plot = st.selectbox("Plot Value", options=['Amplitude', 'Real', 'Imaginary', 'Phase'], key="tab5_vtoplot")
            vmax_2d = st.number_input("vmax (Plot)", value=3.0, step=0.5, key="tab5_vmax")
            
            default_vmin = -vmax_2d if value_to_plot in['Real', 'Imaginary', 'Phase'] else 0.0
            vmin_2d = st.number_input("vmin (Plot)", value=float(default_vmin), step=0.1, key="tab5_vmin")
        ...
        with col_plot:
            ...
            if compute_plot_btn:
                with st.spinner("Computing 2D FFT..."):
                    scan.do_2dfft(data=scan.windowed, window=None, 
                                  subtract_baseline=st.session_state['fft2d_sub_baseline'], 
                                  Nt=st.session_state['fft2d_Nt'], 
                                  Ntau=st.session_state['fft2d_Ntau'])
                    st.session_state.scan = scan
                    
                with st.spinner("Rendering Interactive 2D FFT..."):
                    fig = plotly_2d_fft(scan.ft_data, scan.freq_t, scan.freq_tau, title='2D FFT', 
                                        vmin=vmin_2d, vmax=vmax_2d, cmap=cmap_2d, 
                                        log_scale=log_scale, value_to_plot=value_to_plot, plot_guidelines=True)
                    st.plotly_chart(fig, use_container_width=True)
            
            elif hasattr(scan, 'ft_data') and scan.ft_data is not None:
                fig = plotly_2d_fft(scan.ft_data, scan.freq_t, scan.freq_tau, title='2D FFT', 
                                    vmin=vmin_2d, vmax=vmax_2d, cmap=cmap_2d, 
                                    log_scale=log_scale, value_to_plot=value_to_plot, plot_guidelines=True)
                st.plotly_chart(fig, use_container_width=True)
            ...
            st.divider()
            if st.button("💾 Save Parameters to Config", use_container_width=True):
                base_dir = os.path.dirname(st.session_state.base_name)
                config_path = os.path.join(base_dir, "analysis_config.json")
                ...
                try:
                    ...
                    st.success(f"Saved successfully to `{os.path.basename(config_path)}` in the dataset directory!")
                except Exception as e:
...
with tab6:
    ...
    if st.session_state.scan is None or getattr(st.session_state.scan, 'ft_data', None) is None:
        ...
        with col_ctrl_cs:
            ...
            cs_cmap = st.selectbox("2D Map Colormap", options=['magma', 'jet', 'viridis', 'cividis', 'inferno', 'blackbody', 'bwr', 'icefire'], index=0)
            cs_vmax = st.number_input("2D Map vmax", value=3.0, step=0.5)
            default_cs_vmin = -cs_vmax if cs_y_axis in ['Real', 'Imaginary', 'Phase'] else 0.0
            cs_vmin = st.number_input("2D Map vmin", value=float(default_cs_vmin), step=0.1)

        with col_plot_cs:
            scan = st.session_state.scan
            
            # Top Plot: The 2D Map with Overlay Lines
            ...
            st.plotly_chart(fig_2d_cs, use_container_width=True)
            
            # Bottom Plot: 1D Cross Sections
            fig_1d_cs = plotly_cross_sections(scan, st.session_state.cs_lines, cs_x_axis, cs_y_axis)
            st.plotly_chart(fig_1d_cs, use_container_width=True)